...
<div class="container mt-4">
    ...
    <form method="GET" class="row g-2 mb-3 align-items-center">
        <div class="col-md-5">
            <input type="text" name="search" class="form-control" placeholder="Tìm kiếm theo tên hoặc mã SV" value="{{ $search }}">
        </div>
        <div class="col-md-3">
            <select name="status" class="form-select">
                <option value="">-- Tất cả sinh viên --</option>
                <option value="checked" {{ request('status') == 'checked' ? 'selected' : '' }}>✅ Đã điểm danh</option>
                <option value="unchecked" {{ request('status') == 'unchecked' ? 'selected' : '' }}>⚠️ Chưa điểm danh</option>
            </select>
        </div>
        <div class="col-md-2">
                ...
                <tr>
                    ...
                    <td>
                        @if(!empty($s->image_url))
                            <img src="{{ $s->image_url }}" class="rounded" style="width:70px;height:70px;object-fit:cover;">
                        @else
                    ...
                    <td>
                        ...
                        <input type="hidden" name="evaluations[{{ $loop->index }}][event_id]" value="{{ $event->event_id }}">
                        <input type="hidden" name="evaluations[{{ $loop->index }}][user_id]" value="{{ $s->user_id }}">
                    </td>
                    <td>
