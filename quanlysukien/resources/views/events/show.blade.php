...
@section('content')
    ...
    @php
        ...
        $created = $event->created_at
            ? ($event->created_at instanceof \Carbon\Carbon
                ? $event->created_at
        ...
        $registeredCount = DB::table('event_registration')->where('event_id', $event->event_id)->count();
    @endphp
    <div class="container py-4" style="max-width:980px">
        ...
        <div class="card shadow-sm border-0 overflow-hidden">
            <div class="ratio ratio-21x9 bg-light">
                <img src="{{ $event->image_url ?: 'https://picsum.photos/seed/ongoing' . $event->event_id . '/400/220' }}"
                    class="card-img-top" alt="Ảnh sự kiện {{ $event->event_name }}" style="object-fit: cover;">
            ...
            <div class="card-body p-4">
                ...
                <div class="d-flex flex-wrap gap-2 mb-3">
                    ...
                    @if (!is_null($event->max_participants))
                        <span class="chip"><i class="bi bi-people me-1"></i> Tối đa:
                            {{ $event->max_participants }}</span>
                    @endif
                    @auth
                        @if (auth()->user()->role === 'admin')
                            @if (!is_null($event->max_participants))
                                <span class="chip" role="button" data-event-id="{{ $event->event_id }}" id="view-registered"
                    ...
                    @if ($created)
                        <span class="chip"><i class="bi bi-clock-history me-1"></i> Tạo:
                            {{ $created->format('d/m/Y H:i') }}</span>
                    @endif
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function loadList(eventId, cls = '', fac = '') {
                ...
                try {
                    ...
                    const fetchUrl =
                        `${baseUrl}/${eventId}/registrations?class=${encodeURIComponent(cls)}&faculty=${encodeURIComponent(fac)}`;
            ...
            `;
                    ...
                    if (!students.length) {
                        listDiv.innerHTML = filterHtml +
                            `<p class="text-center text-muted">Chưa có sinh viên nào đăng ký.</p>`;
            ...
            `;
                ...
                `;
                    ...
                    listDiv.innerHTML =
                        `<p class="text-danger text-center">❌ Lỗi tải dữ liệu: ${err.message}</p>`;
                }
            }
