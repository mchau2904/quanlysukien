                    ...
                    <div class="collapse navbar-collapse" id="navbarNav">
                        ...
                        <ul class="navbar-nav">
                            ...
                            @auth
                                @if (auth()->user()->role === 'student')
                                    <li class="nav-item">
                                        <a class="nav-link {{ request()->routeIs('registrations.mine') ? 'active' : '' }}"
                                            href="{{ route('registrations.mine') }}">
                                            Sự kiện của tôi
                                        </a>
                                    </li>
                                @endif
                            ...
                            @auth
                                @if (auth()->user()->role === 'admin')
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link {{ request()->routeIs('evaluation.*') ? 'active' : '' }}"
            ...
            <div class="col-md-3 text-end">
                            ...
                            @php
                                ...
                                $unreadCount = \DB::table('notifications as n')
                                    ->where(function ($q) use ($userId) {
                                        $q->where('n.user_id', $userId)->orWhereNull('n.user_id');
                                    })
                ...
                @else
                    <div class="d-inline-block me-3">
                        <button class="btn btn-thong-bao" onclick="window.location.href='{{ route('login') }}'">
                            <i class="bi bi-bell"></i> Thông Báo
                        </button>
                    </div>
                @endauth
                    ...
                    <div class="dropdown d-inline-block">
                        <button class="btn p-0 border-0 bg-transparent" data-bs-toggle="dropdown">
                            <img src="{{ auth()->user()->avatar_url ?? 'https://www.gravatar.com/avatar/' . md5(strtolower(trim(auth()->user()->email ?? 'no@local'))) . '?s=80&d=identicon' }}"
                                class="rounded-circle shadow-sm" style="width:36px;height:36px;object-fit:cover"
                                alt="avatar">
                        </button>
                        <ul class="dropdown-menu dropdown-menu-end shadow" style="min-width:240px;border-radius:12px;">
                            ...
                            @auth
                                @if (auth()->user()->role === 'student')
                                    <li><a class="dropdown-item" href="{{ route('registrations.mine') }}">Sự kiện của tôi</a>
                                    </li>
                                @endif
                            @endauth
                            <li>
                ...
                @else
                    <a href="{{ route('login') }}" class="btn btn-dang-xuat">Đăng nhập</a>
                @endauth
                            ...
                            .then(({
                                ...
                                import(
                                    ...
                                    .then(({
                                        ...
                                        if (currentUserId) {
                                            const notifyRef = ref(db,
                                                `Notifications/${currentUserId}`);
                                            get(notifyRef).then(snapshot => {
                                                if (snapshot.exists()) {
                                                    snapshot.forEach(child => {
                                                        remove(ref(db,
                                                            `Notifications/${currentUserId}/${child.key}`
                                                        ));
                                                    });
                                                }
                                            });
